...
#include "../Cfg/Device_Config.h"
#include <Arduino.h>


#ifdef SENSOR_AHT11
#include "../Sensors/Sens_AHT11.h"    // HB-UNI-Sensor1 custom sensor class
#endif
#ifdef SENSOR_LDR
#include "../Sensors/Sens_LDR.h"   // HB-UNI-Sensor1 custom sensor class
#endif
...
const struct DeviceInfo PROGMEM devinfo = {
    cDEVICE_ID,
    cDEVICE_SERIAL,
    { 0xF4, 0x02 },    // Device Model - fits to hb-uni-sensor-THPL-AHT11.xml
                       // <parameter index="10.0" size="2.0" const_value="0xF402" name="Device Model" />
    0x10,
...
public:
    void init(uint8_t msgcnt, int16_t temp, uint16_t my_humidity10, uint16_t operatingVoltage1000, bool batLow, uint16_t lux)
    {
        ...
        Message::init(0x11, msgcnt, 0x70, flags, t1, t2); // length 17 = 0x11 (6 bytes payload: humidity, voltage, lux)
...
public:
    ...
    virtual void trigger(AlarmClock& clock)
    {
        ...
        msg.init(msgcnt, temperature10, humidity10, operatingVoltage1000, device().battery().low(), lux);
    ...
    void measure()
    {
...
#ifdef SENSOR_AHT11
        AHT11.measure();
        temperature10 = AHT11.temperature();
        humidity10    = AHT11.humidity();
#endif
        ...
        int16_t tempInt = temperature10 / 10;
        uint8_t tempDec = (uint8_t)abs(temperature10 % 10);
        ...
        itoa(humidity10 / 10, humidityStr, 10);

        // Format battery voltage with two decimal places (e.g. "3.12")
        itoa(operatingVoltage1000 / 1000, batterieStr, 10);
        strcat(batterieStr, ".");
...
#ifdef SENSOR_AHT11
        AHT11.init((int8_t)this->getList1().tempOffset10(), (int8_t)this->getList1().humidOffset10());
#endif
#ifdef SENSOR_LDR
        LDR.init();
#endif
